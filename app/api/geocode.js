...
module.exports=async function handler(req,res){
  ...
  try{
    ...
    if(suggestions){
      const result={suggestions:places.map(place=>({lat:Number(place.lat),lon:Number(place.lon),label:String(place.display_name||query)})).filter(place=>Number.isFinite(place.lat)&&Number.isFinite(place.lon))};
