...
function compactLabel(place,fallback){
  ...
  const parts=[locality,region,address.country,address.postcode]
    .map(value=>String(value||'').trim()).filter((value,index,items)=>value&&items.indexOf(value)===index);
...
module.exports=async function handler(req,res){
  ...
  try{
    ...
    url.search=new URLSearchParams({format:'jsonv2',q:query,limit:suggestions?'5':'1',addressdetails:'1'}).toString();
    ...
    if(suggestions){
      const result={suggestions:places.map(place=>({lat:Number(place.lat),lon:Number(place.lon),label:compactLabel(place,query)})).filter(place=>Number.isFinite(place.lat)&&Number.isFinite(place.lon))};
