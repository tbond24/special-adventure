(() => {
  ...
  renderAccount=function(){
    ...
    const distance=distanceUnit(),theme=document.documentElement.dataset.theme||'dark';
    ...
    const currencies=Object.values(MARKETS).map(item=>item.currency).filter((value,index,array)=>array.indexOf(value)===index);
    layout(`<section class="account-page"><div class="section-head"><div><h1>You</h1><p class="muted">${escapeHtml(currentUser.email||'Signed-in Vacancy member')}</p></div></div><section class="settings-group"><h2>Preferences</h2>${row('settingTheme','Theme',theme==='dark'?'Dark':'Light')}<label class="settings-row settings-select"><span><strong>Display currency</strong><small>Prices convert without moving your search</small></span><select id="settingCurrency" aria-label="Display currency in settings">${currencies.map(code=>`<option ${code===displayCurrency?'selected':''}>${code}</option>`).join('')}</select></label>${row('settingDistance','Distance units',distance==='km'?'Kilometres':'Miles')}</section><section class="settings-group"><h2>Your activity</h2>${row('yourListings','Properties and listings','Manage properties, units and availability')}${row('listRoom','List a unit','Add to a new or existing property')}</section><section class="settings-group"><h2>Privacy and security</h2>${row('blockedAccounts','Blocked accounts','Review and unblock members')}${row('accountSecurity','Password and sessions','Reset password or sign out every session')}${row('adminEntry','Admin','Available only to authorised operators')}</section><section class="settings-group settings-danger"><button class="settings-row" id="signout"><span><strong>Sign out</strong></span></button><button class="settings-row" id="deleteEntry"><span><strong>Delete account</strong><small>Permanently remove your Vacancy account</small></span></button></section></section><div class="sheet-backdrop" id="accountSheetBackdrop" hidden></div><section class="action-sheet account-sheet" id="accountSheet" hidden role="dialog" aria-modal="true" aria-labelledby="accountSheetTitle"></section>`);
    document.querySelector('#settingTheme').onclick=()=>{setVacancyTheme(theme==='dark'?'light':'dark');renderAccount()};
    document.querySelector('#settingCurrency').onchange=async event=>{await setDisplayCurrency(event.target.value);if(parseHash().name==='account')renderAccount()};
