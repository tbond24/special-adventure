(() => {
  ...
  function setupUnitPhotoChoices(form,unit,files=filePools.get(form)||[]){
    ...
    body.innerHTML=files.map((file,index)=>`<button type="button" data-pool-index="${index}" class="${selectedPool.has(file)?'selected':''}">${escapeHtml(file.name)}</button>`).join('')+extras.map((file,index)=>`<span class="unit-extra-photo">${escapeHtml(file.name)}<button type="button" data-remove-unit-photo="${index}" aria-label="Remove ${escapeHtml(file.name)}">×</button></span>`).join('');choices.querySelector('summary span').textContent=`${selected.length} selected`;
