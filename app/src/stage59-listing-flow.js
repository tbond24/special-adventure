(() => {
  ...
  function applyListingType(form,type){
    const map={Room:{unit:'Room',property:'House'},Studio:{unit:'Studio',property:'Apartment'},Apartment:{unit:'1 bedroom',property:'Apartment'},House:{unit:'House',property:'House'},Shop:{unit:'Shop',property:'Shop'}}[type]||{unit:type,property:type};
    const unit=form.querySelector('[data-base-name="unitType"], [name="unitType"]'),property=form.querySelector('[name="propertyType"]');
    if(unit&&map.unit){if(![...unit.options].some(option=>option.value===map.unit))unit.add(new Option(map.unit,map.unit));unit.value=map.unit;unit.dispatchEvent(new Event('change',{bubbles:true}))}
    if(property&&map.property){if(![...property.options].some(option=>option.value===map.property))property.add(new Option(map.property,map.property));property.value=map.property;property.dispatchEvent(new Event('change',{bubbles:true}))}
  }
  ...
  function installListingStart(host){
    ...
    start.onclick=event=>{const typeButton=event.target.closest('[data-listing-type]');if(typeButton){selected=typeButton.dataset.listingType;start.querySelectorAll('[data-listing-type]').forEach(button=>button.classList.toggle('selected',button===typeButton));start.querySelector('.listing-add-choice').hidden=false;return}const mode=event.target.closest('[data-add-mode]')?.dataset.addMode;if(!mode||!selected)return;let resolvedType=selected;if(selected==='Other'){resolvedType=start.querySelector('.custom-listing-type input')?.value.trim()||'';if(!resolvedType){start.querySelector('.custom-listing-type input')?.focus();return}}let form;if(mode==='new'){const create=initial[0]?.querySelector('#createProperty');if(create)create.click();else initial.forEach(node=>node.hidden=false);form=host.querySelector('#listingForm');if(form){applyListingType(form,resolvedType);addAddressSuggestions(form)}}else{initial.forEach(node=>node.hidden=false);form=host.querySelector('#existingListingForm');if(form){applyListingType(form,resolvedType);form.hidden=false}}if(!start.isConnected)host.prepend(start);start.classList.add('complete');start.querySelectorAll('button').forEach(button=>button.disabled=true);form?.scrollIntoView({behavior:'smooth',block:'start'})};
  }
