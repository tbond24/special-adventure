...
function syncRadiusUI(){const label=document.querySelector('#radiusLabel'),status=document.querySelector('#radiusStatus'),clear=document.querySelector('#clearLocation'),radius=document.querySelector('#radius');if(label)label.textContent=`${radiusValue} ${market().distanceUnit}`;if(status)status.textContent=searchCenter?`Filtering within ${radiusValue} ${market().distanceUnit} of your chosen centre.`:'Choose a point on the map to use radius.';if(clear)clear.disabled=!searchCenter;if(radius)radius.disabled=!searchCenter;updateMapRadius()}
...
function layout(content){document.querySelector('#app').innerHTML=`${content}<footer class="site-footer"><a href="#privacy">Privacy</a><a href="#terms">Terms</a><a href="#storage">Storage</a><a href="#safety">Safety</a><span>© ${new Date().getFullYear()} Vacancy</span></footer>`;bindHeader()}
