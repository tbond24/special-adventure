window.VACANCY_BACKEND = (() => {
  ...
  async function activeVacancies() {
    const select = encodeURIComponent('id,rent_amount,rent_currency,rent_period,monthly_rent,deposit,bills_included,available_from,minimum_stay_weeks,confirmed_at,updated_at,expires_at,status,rooms!inner(id,name,room_type,unit_type,furnished,ensuite,furnished_known,ensuite_known,max_occupants,smoking_allowed_override,pets_considered_override,description,media(id,storage_path,sort_order),properties!inner(id,title,suburb,city,state,postcode,country,market_code,property_type,parking_spaces,pets_considered,smoking_allowed,household_summary,landmark,water_available,electricity_available,security_available,internet_available,custom_features,public_latitude,public_longitude,owner_id,profiles!properties_owner_id_fkey(id,display_name,bio,avatar_path)))');
  ...
  async function mfaFactors(){const data=await mfaRequest('factors');const all=[...(data?.totp||[]),...(data?.phone||[]),...(data?.factors||[])];return all.filter((factor,index)=>factor?.id&&all.findIndex(item=>item.id===factor.id)===index)}
  async function mfaEnroll(){return mfaRequest('factors',{method:'POST',body:JSON.stringify({factor_type:'totp',friendly_name:'Vacancy authenticator'})})}
  async function mfaVerify(factorId,code){const challenge=await mfaRequest(`factors/${encodeURIComponent(factorId)}/challenge`,{method:'POST',body:'{}'});const data=await mfaRequest(`factors/${encodeURIComponent(factorId)}/verify`,{method:'POST',body:JSON.stringify({challenge_id:challenge.id,code:String(code||'').replace(/\s/g,'')})});if(data?.access_token)saveSession(data);return data}
  async function mfaUnenroll(factorId){return mfaRequest(`factors/${encodeURIComponent(factorId)}`,{method:'DELETE'})}
  async function adminMembership(){const u=await currentUser();if(!u||u.is_anonymous)return false;const rows=await rest(`admin_users?select=user_id&user_id=eq.${encodeURIComponent(u.id)}&limit=1`);return rows.length===1}
  ...
  async function createListing(input){
    ...
    body.p_public_latitude=Number(input.publicLatitude);
    body.p_public_longitude=Number(input.publicLongitude);
    const rows=await rest('rpc/create_vacancy_listing_v4',{method:'POST',body:JSON.stringify(body)});
    return Array.isArray(rows)?rows[0]:rows;
  }

  async function listingForEdit(id){
    const u=await currentUser(); if(!u)throw new Error('Sign in first');
    const select=encodeURIComponent('id,rent_amount,rent_currency,rent_period,monthly_rent,deposit,bills_included,available_from,minimum_stay_weeks,status,rooms!inner(id,name,unit_type,furnished,ensuite,furnished_known,ensuite_known,max_occupants,smoking_allowed_override,pets_considered_override,description,media(id,storage_path,mime_type,sort_order,status),properties!inner(id,suburb,city,state,postcode,country,market_code,landmark,property_type,parking_spaces,pets_considered,smoking_allowed,water_available,electricity_available,security_available,internet_available,public_latitude,public_longitude,household_summary,owner_id))');
  ...
  async function setPropertyFeatures(propertyId,features){return rest('rpc/set_property_custom_features',{method:'POST',body:JSON.stringify({p_property_id:propertyId,p_features:features||[]})});}
  async function signInGuest(displayName){
    const data=await parse(await fetch(`${URL}/auth/v1/signup`,{method:'POST',headers:baseHeaders,body:JSON.stringify({data:{display_name:String(displayName||'Guest').trim().slice(0,80)}})}));
    saveSession(data);return data;
  }
  ...
  async function adminOperationalHealth(days=7){return rest('rpc/admin_operational_health',{method:'POST',body:JSON.stringify({p_days:Number(days)})})}
  ...
  async function uploadListingImages(vacancyId, files, uploadKey='manual'){
    ...
    for(let i=0;i<list.length;i++){
      ...
      const existing=await rest(`media?select=id,storage_path&owner_id=eq.${u.id}&room_id=eq.${roomId}&storage_path=eq.${encodeURIComponent(path)}`);
  ...
  async function conversations(){
    const select=encodeURIComponent('id,requested_move_in,stay_weeks,renter_intro,created_at,vacancies!inner(id,monthly_rent,rooms!inner(name,properties!inner(suburb,owner_id))),conversation_members(user_id,last_read_at),messages(id,body,sender_id,created_at)');
  ...
  async function markConversationRead(conversationId){const u=await currentUser();if(!u)return;return rest(`conversation_members?conversation_id=eq.${conversationId}&user_id=eq.${u.id}`,{method:'PATCH',body:JSON.stringify({last_read_at:new Date().toISOString()})});}
  async function profile(){const u=await currentUser();if(!u)throw new Error('Sign in first');const [rows,contacts]=await Promise.all([rest(`profiles?select=id,display_name,bio,avatar_path,phone_verified,created_at&id=eq.${u.id}`),rest(`profile_private_contacts?select=phone_number,whatsapp_number,enquiry_in_app,enquiry_email,enquiry_phone,enquiry_whatsapp&user_id=eq.${u.id}`)]);return rows[0]?{...rows[0],email_confirmed:Boolean(u.email_confirmed_at||u.confirmed_at),...(contacts[0]||{})}:null;}
  async function updateProfile(input){const u=await currentUser();if(!u)throw new Error('Sign in first');await rest(`profiles?id=eq.${u.id}`,{method:'PATCH',body:JSON.stringify({display_name:String(input.displayName||'').trim().slice(0,80)})});return rest('profile_private_contacts',{method:'POST',headers:{Prefer:'resolution=merge-duplicates,return=representation'},body:JSON.stringify({user_id:u.id,phone_number:String(input.phoneNumber||'').trim()||null,whatsapp_number:String(input.whatsappNumber||'').trim()||null,updated_at:new Date().toISOString()})});}
  async function updateContactPreferences(input){return rest('rpc/set_contact_preferences',{method:'POST',body:JSON.stringify({p_in_app:Boolean(input.inApp),p_email:Boolean(input.email),p_phone:Boolean(input.phone),p_whatsapp:Boolean(input.whatsapp)})})}
  async function contactOptions(vacancyId){return rest('rpc/vacancy_contact_options',{method:'POST',body:JSON.stringify({p_vacancy_id:vacancyId})})}
  ...
  async function adminAccountHierarchy(userId){return rest('rpc/admin_account_hierarchy',{method:'POST',body:JSON.stringify({p_user_id:userId})});}
