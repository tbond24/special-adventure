window.VACANCY_BACKEND = (() => {
  ...
  async function profile(){const u=await currentUser();if(!u)throw new Error('Sign in first');const [rows,contacts]=await Promise.all([rest(`profiles?select=id,display_name,bio,avatar_path,phone_verified,created_at&id=eq.${u.id}`),rest(`profile_private_contacts?select=phone_number,whatsapp_number&user_id=eq.${u.id}`)]);return rows[0]?{...rows[0],...(contacts[0]||{})}:null;}
