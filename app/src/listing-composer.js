...
function upgradeListingFlow(form,steps,previewButton,previewOutput){
  ...
  let manualMode=false;const map=form.querySelector('#newPropertyMap');if(map){const locationBody=map.parentElement,manual=document.createElement('div');manual.className='manual-location-choice';manual.innerHTML='<span>OR</span><button type="button" class="ghost">Enter address manually</button>';locationBody.after(manual);const locationNames=['region','city','locality','landmark','postal','address'];const fields=locationNames.map(name=>form.querySelector(`[name="${name}"]`)?.closest('label')).filter(Boolean);fields.forEach(field=>field.classList.add('manual-location-field','manual-location-hidden'));manual.querySelector('button').onclick=()=>{const hidden=fields.some(field=>field.classList.contains('manual-location-hidden'));manualMode=hidden;fields.forEach(field=>field.classList.toggle('manual-location-hidden',!hidden));manual.querySelector('button').textContent=hidden?'Use map selection':'Enter address manually'}}

  const headings=[...form.querySelectorAll('h2')],location=headings.find(x=>x.textContent==='Location'),property=headings.find(x=>x.textContent==='Property'),units=headings.find(x=>x.textContent.includes('Unit'));
  const wrap=(heading,boundary,title)=>{if(!heading)return null;const details=document.createElement('details'),summary=document.createElement('summary'),body=document.createElement('div');details.className='composer-section wide';summary.innerHTML=`<span class="section-state" aria-hidden="true"></span><strong>${title}</strong><small>Tap to review</small>`;body.className='composer-section-body form-grid';heading.before(details);details.append(summary,body);let node=heading.nextElementSibling;heading.remove();while(node&&node!==boundary){const next=node.nextElementSibling;body.append(node);node=next}return details};
