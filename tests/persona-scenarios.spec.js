...
async function publishListing(page, roomName, options = {}) {
  ...
  await expect(page.locator('#mine').getByText(roomName, { exact: true })).toBeVisible();
}
...
test('L5 Njeri — live-in owner receives and answers a renter enquiry', async ({ browser }) => {
  ...
  await card.click();
  await seeker.page.getByRole('button', { name: 'Message' }).click();
  ...
  await enquiry.getByRole('button', { name: 'Send message' }).click();
