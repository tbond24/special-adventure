...
test('every page remains contained and readable across themes and viewports',async({page})=>{
  ...
  for(const width of [1280,390,320]){
    ...
    const screens=[
      ...
      ['admin',()=>page.evaluate(async()=>{currentUser={id:'admin-user',email:'admin@example.com'};VACANCY_BACKEND.adminDashboard=async()=>({users_total:1234,properties:20,units:28,active_listings:18,open_reports:1,oldest_report_hours:12,enquiries_7d:8,listings_7d:4,errors_24h:0});VACANCY_BACKEND.adminSearch=async()=>({users:[{id:'user-one',display_name:'Example member',account_status:'active',created_at:'2026-09-01'}],listings:[{id:'listing-one',name:'Long but realistic two bedroom listing name',title:'Example property',suburb:'Kasarani',city:'Nairobi',status:'active'}]});VACANCY_BACKEND.adminReports=async()=>[{id:'report-one',reason:'The address appears to be inaccurate',status:'open',vacancy_id:'listing-one',created_at:'2026-09-07T00:00:00Z'}];VACANCY_BACKEND.adminAuditLog=async()=>[];await renderAdmin()})]
    ];
